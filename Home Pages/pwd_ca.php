...
if(isset($_POST['PwdChangebtn']))
{
    ...
    $rollno_query = "SELECT * FROM classadvisor WHERE password='$currentpwd' and username='$username' ";
    $rollno_query_run = mysqli_query($db, $rollno_query);
    if(mysqli_num_rows($rollno_query_run) > 0)
    {
        $query = "UPDATE classadvisor SET password='$newpassword' where username='$username' ";
        $query_run = mysqli_query($db, $query);

        if($query_run){
            $_SESSION['status'] = "Your password is Updated";
            $_SESSION['status_code'] = "success";
            $MSG = 'Location: CA_Pwd.php';
            header($MSG); 
        }
        else{
